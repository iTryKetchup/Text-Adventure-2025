    ...
    while True:
        ...
        if command.startswith('go '):
            ...
            if 'enemy_on_inspect' in rooms[current_room] and 'enemy' not in rooms[current_room]:
                enemy = str(rooms[current_room]['enemy_on_inspect']).lower()
            ...
            if item in inventory:
                if item == 'flashlight':
                    key = None
                    if 'flashlight_descriptions' in rooms[current_room]:
                        key = 'flashlight_descriptions'
                    elif 'flashlight_description' in rooms[current_room]:
                        key = 'flashlight_description'
                    if key:
                        print("You turn on the flashlight, revealing:")
                        for direction, desc in rooms[current_room][key].items():
                            print(f"To the {direction}, {desc}")
                    else:
                        print("You shine the flashlight around, but nothing new is revealed.")
                elif 'enemy' in rooms[current_room]:
                    enemy = str(rooms[current_room]['enemy']).lower()
                    if (item == 'knife' and enemy == 'vampire') or \
                       (item == 'bat' and enemy == 'zombie') or \
                       (item in ['rifle', 'pistol'] and enemy in ['vampire', 'zombie']):
